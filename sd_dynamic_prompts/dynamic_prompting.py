...
def _get_hr_mode_details(p, fallback_hr_prompt: str) -> tuple[str, str, bool]:
    hires_prompt_mode = "Default"
    original_hr_prompt = fallback_hr_prompt
    remove_fp_extra_networks = False

    mode_info = getattr(p, "extra_generation_params", {}).get("Hires prompt mode")
    if not mode_info:
        return hires_prompt_mode, original_hr_prompt, remove_fp_extra_networks

    try:
        from hires_fix_tweaks.hr_modules.hr_prompt_mode import parse_mode_info

        mode_p, hr_prompt, _, _, _, _, _, remove_fp_extra_networks = parse_mode_info(mode_info)
        if mode_p:
            hires_prompt_mode = mode_p
        if hr_prompt is not None:
            original_hr_prompt = hr_prompt
    except Exception:
        logger.exception("Failed to parse Hires prompt mode infotext")
...
def _get_hr_fix_prompts(
    ...
    original_prompt: str,
    hires_prompt_mode: str = "Default",
    remove_fp_extra_networks: bool = False,
    ...
    if use_raw_prompt:
        base_prompts = repeat_iterable_to_length([original_prompt], prompt_count)

    if remove_fp_extra_networks:
        base_prompts = [_remove_extra_networks(prompt) for prompt in base_prompts]

    if not original_hr_prompt:
        return base_prompts
    ...
    if hires_prompt_mode == "Append":
        ...
        return [
            ...
            for base_prompt, hires_prompt in zip(base_prompts, hr_prompts)
        ]
...
class Script(scripts.Script):
    ...
    def process(
        ...
        if hr_fix_enabled and hasattr(p, "all_hr_prompts"):
            ...
            hires_prompt_mode, original_hr_prompt, remove_fp_extra_networks = _get_hr_mode_details(
                p,
                original_hr_prompt,
            )
        else:
        ...
        if hr_fix_enabled:
            hr_prompt_raw = getattr(p, "hr_prompt_raw", False)
            hr_prompt_seeds = None if unlink_seed_from_prompt else p.all_seeds
            p.all_hr_prompts = _get_hr_fix_prompts(
                all_prompts,
                original_hr_prompt,
                original_prompt,
                hires_prompt_mode=hires_prompt_mode,
                remove_fp_extra_networks=remove_fp_extra_networks,
